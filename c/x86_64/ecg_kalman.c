...
int preprocess_ecg_data(float **data, int rows, float **preprocessed_output, int *ecg_complex_length) {
    ...
    /* Notch filter centered around 60Hz (50Hz for rest) for powerline/baseline wander */
    BWBandStop* bs_filter = create_bw_band_stop_filter(4, (int)sampling_rate, 49, 61);
    for (int i = 0; i < rows; i++) {
    ...
    for (int i = 0; i < rows; i++) {
        in[i] = (double)preprocessed_output[i][0];
    }
